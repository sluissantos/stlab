...
bool begin(i2c_port_t port_num, uint8_t addr, int sda, int scl){
    __i2c_num = port_num;
    ESP_LOGI(TAG_XPOWER, "Using ESP-IDF Driver interface.");
    if (__has_init)return initImplXPower();
    __sda = sda;
    __scl = scl;
    __addr = addr;
...
bool begin(uint8_t addr, iic_fptr_t readRegCallback, iic_fptr_t writeRegCallback)
{
    ...
    __addr = addr;
    return initImplXPower();
}
*/

int readRegisterDataBuffer(uint8_t reg, uint8_t *buf, uint8_t length)
{
...
int writeRegisterDataBuffer(uint8_t reg, uint8_t *buf, uint8_t length)
{
...
{
    uint8_t val = 0;
    return readRegisterDataBuffer(reg, &val, 1) == -1 ? -1 : val;
}

int writeRegister(uint8_t reg, uint8_t val)
{
    return writeRegisterDataBuffer(reg, &val, 1);
}
...
uint16_t readRegisterH8L4(uint8_t highReg, uint8_t lowReg)
{
...
uint16_t readRegisterH8L5(uint8_t highReg, uint8_t lowReg)
{
    int h8 = readRegister(highReg);
...
void XPowersAXP2101(uint8_t addr, iic_fptr_t readRegCallback, iic_fptr_t writeRegCallback){
    thisReadRegCallback = readRegCallback;
    thisWriteRegCallback = writeRegCallback;
    __addr = addr;
}
...
bool writeDataBuffer(uint8_t *data, uint8_t size)
{
    ...
    return writeRegisterDataBuffer(XPOWERS_AXP2101_DATA_BUFFER1, data, size);
}

bool readDataBuffer(uint8_t *data, uint8_t size)
{
    if (size > XPOWERS_AXP2101_DATA_BUFFER_SIZE)return false;
    return readRegisterDataBuffer(XPOWERS_AXP2101_DATA_BUFFER1, data, size);
}
...
bool setButtonBatteryChargeVoltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_BTN_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! Button battery charging step voltage is %u mV", XPOWERS_AXP2101_BTN_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_BTN_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The minimum charge termination voltage of the coin cell battery is %u mV", XPOWERS_AXP2101_BTN_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_BTN_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The minimum charge termination voltage of the coin cell battery is %u mV", XPOWERS_AXP2101_BTN_VOL_MAX);
        return false;
    }
...
bool setSysPowerDownVoltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_VSYS_VOL_THRESHOLD_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_VSYS_VOL_THRESHOLD_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_VSYS_VOL_THRESHOLD_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The minimum settable voltage of VSYS is %u mV", XPOWERS_AXP2101_VSYS_VOL_THRESHOLD_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_VSYS_VOL_THRESHOLD_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The maximum settable voltage of VSYS is %u mV", XPOWERS_AXP2101_VSYS_VOL_THRESHOLD_MAX);
        return false;
    }
...
bool setDC1Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_DCDC1_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_DCDC1_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_DCDC1_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! DC1 minimum voltage is %u mV", XPOWERS_AXP2101_DCDC1_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_DCDC1_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! DC1 maximum voltage is %u mV", XPOWERS_AXP2101_DCDC1_VOL_MAX);
        return false;
    }
...
bool setDC2Voltage(uint16_t millivolt)
{
    ...
    if (millivolt >= XPOWERS_AXP2101_DCDC2_VOL1_MIN && millivolt <= XPOWERS_AXP2101_DCDC2_VOL1_MAX) {
        if (millivolt % XPOWERS_AXP2101_DCDC2_VOL_STEPS1) {
            ESP_LOGI(TAG_XPOWER, "Mistake !  The steps is must %umV", XPOWERS_AXP2101_DCDC2_VOL_STEPS1);
            return false;
        }
        ...
        if (millivolt % XPOWERS_AXP2101_DCDC2_VOL_STEPS2) {
            ESP_LOGI(TAG_XPOWER, "Mistake !  The steps is must %umV", XPOWERS_AXP2101_DCDC2_VOL_STEPS2);
            return false;
        }
...
bool setDC3Voltage(uint16_t millivolt)
{
    ...
    if (millivolt >= XPOWERS_AXP2101_DCDC3_VOL1_MIN && millivolt <= XPOWERS_AXP2101_DCDC3_VOL1_MAX) {
        if (millivolt % XPOWERS_AXP2101_DCDC3_VOL_STEPS1) {
            ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %umV", XPOWERS_AXP2101_DCDC3_VOL_STEPS1);
            return false;
        }
        ...
        if (millivolt % XPOWERS_AXP2101_DCDC3_VOL_STEPS2) {
            ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %umV", XPOWERS_AXP2101_DCDC3_VOL_STEPS2);
            return false;
        }
        ...
        if (millivolt % XPOWERS_AXP2101_DCDC3_VOL_STEPS3) {
            ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %umV", XPOWERS_AXP2101_DCDC3_VOL_STEPS3);
            return false;
        }
...
bool setDC4Voltage(uint16_t millivolt)
{
    ...
    if (millivolt >= XPOWERS_AXP2101_DCDC4_VOL1_MIN && millivolt <= XPOWERS_AXP2101_DCDC4_VOL1_MAX) {
        if (millivolt % XPOWERS_AXP2101_DCDC4_VOL_STEPS1) {
            ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %umV", XPOWERS_AXP2101_DCDC4_VOL_STEPS1);
            return false;
        }
        ...
        if (millivolt % XPOWERS_AXP2101_DCDC4_VOL_STEPS2) {
            ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %umV", XPOWERS_AXP2101_DCDC4_VOL_STEPS2);
            return false;
        }
...
bool setDC5Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_DCDC5_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_DCDC5_VOL_STEPS);
        return false;
    }
    if (millivolt != XPOWERS_AXP2101_DCDC5_VOL_1200MV && millivolt < XPOWERS_AXP2101_DCDC5_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! DC5 minimum voltage is %umV ,%umV", XPOWERS_AXP2101_DCDC5_VOL_1200MV, XPOWERS_AXP2101_DCDC5_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_DCDC5_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! DC5 maximum voltage is %umV", XPOWERS_AXP2101_DCDC5_VOL_MAX);
        return false;
    }
...
bool setALDO1Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_ALDO1_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_ALDO1_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_ALDO1_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! ALDO1 minimum output voltage is  %umV", XPOWERS_AXP2101_ALDO1_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_ALDO1_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! ALDO1 maximum output voltage is  %umV", XPOWERS_AXP2101_ALDO1_VOL_MAX);
        return false;
    }
...
bool setALDO2Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_ALDO2_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_ALDO2_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_ALDO2_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! ALDO2 minimum output voltage is  %umV", XPOWERS_AXP2101_ALDO2_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_ALDO2_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! ALDO2 maximum output voltage is  %umV", XPOWERS_AXP2101_ALDO2_VOL_MAX);
        return false;
    }
...
bool setALDO3Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_ALDO3_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_ALDO3_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_ALDO3_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! ALDO3 minimum output voltage is  %umV", XPOWERS_AXP2101_ALDO3_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_ALDO3_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! ALDO3 maximum output voltage is  %umV", XPOWERS_AXP2101_ALDO3_VOL_MAX);
        return false;
    }
...
bool setALDO4Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_ALDO4_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_ALDO4_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_ALDO4_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! ALDO4 minimum output voltage is  %umV", XPOWERS_AXP2101_ALDO4_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_ALDO4_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! ALDO4 maximum output voltage is  %umV", XPOWERS_AXP2101_ALDO4_VOL_MAX);
        return false;
    }
...
bool setBLDO1Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_BLDO1_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_BLDO1_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_BLDO1_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! BLDO1 minimum output voltage is  %umV", XPOWERS_AXP2101_BLDO1_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_BLDO1_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! BLDO1 maximum output voltage is  %umV", XPOWERS_AXP2101_BLDO1_VOL_MAX);
        return false;
    }
...
bool setBLDO2Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_BLDO2_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_BLDO2_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_BLDO2_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! BLDO2 minimum output voltage is  %umV", XPOWERS_AXP2101_BLDO2_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_BLDO2_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! BLDO2 maximum output voltage is  %umV", XPOWERS_AXP2101_BLDO2_VOL_MAX);
        return false;
    }
...
bool setCPUSLDOVoltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_CPUSLDO_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_CPUSLDO_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_CPUSLDO_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! CPULDO minimum output voltage is  %umV", XPOWERS_AXP2101_CPUSLDO_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_CPUSLDO_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! CPULDO maximum output voltage is  %umV", XPOWERS_AXP2101_CPUSLDO_VOL_MAX);
        return false;
    }
...
bool setDLDO1Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_DLDO1_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_DLDO1_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_DLDO1_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! DLDO1 minimum output voltage is  %umV", XPOWERS_AXP2101_DLDO1_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_DLDO1_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! DLDO1 maximum output voltage is  %umV", XPOWERS_AXP2101_DLDO1_VOL_MAX);
        return false;
    }
...
bool setDLDO2Voltage(uint16_t millivolt)
{
    if (millivolt % XPOWERS_AXP2101_DLDO2_VOL_STEPS) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! The steps is must %u mV", XPOWERS_AXP2101_DLDO2_VOL_STEPS);
        return false;
    }
    if (millivolt < XPOWERS_AXP2101_DLDO2_VOL_MIN) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! DLDO2 minimum output voltage is  %umV", XPOWERS_AXP2101_DLDO2_VOL_MIN);
        return false;
    } else if (millivolt > XPOWERS_AXP2101_DLDO2_VOL_MAX) {
        ESP_LOGI(TAG_XPOWER, "Mistake ! DLDO2 maximum output voltage is  %umV", XPOWERS_AXP2101_DLDO2_VOL_MAX);
        return false;
    }
...
{
    xpowers_thermal_t threshold = XPOWERS_AXP2101_THREMAL_60DEG; // Defina um valor padrão ou adequado aqui

    // Seu código existente
    threshold = (xpowers_thermal_t)(readRegister(XPOWERS_AXP2101_THE_REGU_THRES_SET) & 0x03);
...
bool disablePowerOutput(uint8_t channel)
{
    if (getProtectedChannel(channel)) {
        ESP_LOGI(TAG_XPOWER, "Failed to disable the power channel, the power channel has been protected");
        return false;
    }
    switch (channel) {
    case XPOWERS_DCDC1:
...
{
    if (getProtectedChannel(channel)) {
        ESP_LOGI(TAG_XPOWER, "Failed to set the power channel, the power channel has been protected");
        return false;
    }
    switch (channel) {
    case XPOWERS_DCDC1:
...
bool setInterruptImpl(uint32_t opts, bool enable)
{
    ...
    //ESP_LOGI(TAG_XPOWER, "%s - HEX:0x%08x.", enable ? "ENABLE" : "DISABLE", opts);
    if (opts & 0x0000FF) {
        value = opts & 0xFF;
        // log_d("Write INT0: %x\n", value);
...
void xpowerInit(void){
    i2c_port_t port_num = (i2c_port_t)0;
    bool result = begin(port_num, AXP2101_SLAVE_ADDRESS, I2C_SDA, I2C_SCL);

    if(!result){
        ESP_LOGE(TAG_XPOWER, "Falha ao iniciar PMU");
    }

    ESP_LOGI(TAG_XPOWER, "getID:0x%x", getChipID());
    ...
    ***********************************/
    ...
    setDC3Voltage(3000);    //SIM7080 Modem main power channel 2700~ 3400V
    enableDC3();

    //Modem GPS Power channel
    setBLDO2Voltage(3300);
    enableBLDO2();      //The antenna power must be turned on to use the GPS function
